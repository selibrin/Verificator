...
vector<int> AdviceLine(int x1, int y1, int x2, int y2, Matrix& a) // функция которая ищет параметры при которых меньше несовпадений, проверяя соседние точки
{
	...
	int limit = CountLine(x1, y1, x2, y2, a, x); // считаю несвопадания при параметрах из файла и дальше проверяю, можно ли их как то улучшить, передвинув точки начала и конца в соседние
	vector<int> v; // сюда запишутся оптимальные координаты 

	int deltaX1 = -1, deltaX2 = -1, deltaY1 = -1, deltaY2 = -1; // начальные смещения каждой точки
	...
	while (deltaX1 <= 1) {
		...
		run(deltaX1, deltaY1, deltaX2, deltaY2); // проход смещений от -1 до +1

		x.clear(); // чистим нашу временную матрицу, т.к. на каждой итерации в ней строиться фигура
	}
	v.push_back(q); v.push_back(w); v.push_back(e); v.push_back(r); // запись оптимальных координат
	return v;
}
...
void Recomendations(const char* filename, int n, int m, double limit, Matrix a)
{
	...
	if (in.is_open() && out.is_open())
	{
		while (!in.eof())
		{
			...
			{
				vector<int> v(4); // оптимальные параметры
			...
			{
				vector<int> v(3); // оптимальные параметры
				...
				if (CountCircle(vec[0], vec[1], vec[2], x, a) < limit) //если точность меньше минимальной точности, ищем лучшие параметры
					AdviceCircle(vec[0], vec[1], vec[2], a);
